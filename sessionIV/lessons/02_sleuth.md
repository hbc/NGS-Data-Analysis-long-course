...
When performing differential expression of genes using count-based methods, multimapping reads were ignored during analysis. However, to analyze the differential expression of gene isoforms, it is expected that RNA-Seq reads will often align to multiple isoforms of the same gene. Therefore, the **multimapping reads cannot be ignored** to properly analyze differential expression of genes isoforms. Due to statistical procedure required to assign reads to gene isoforms in addition to the random processes underlying RNA-Seq, there will be variability in the abundance estimates [[2](https://rawgit.com/pachterlab/sleuth/master/inst/doc/intro.html), [3](http://biorxiv.org/content/biorxiv/early/2016/06/10/058164.full.pdf)]. For example, if we performed multiple technical replicates of an RNA-Seq sample and estimated abundances for gene isoforms, the abundance estimates for the technical replicates would exhibit variability greater than expected. Therefore, we would need technical replicates to account for this technical variation.
